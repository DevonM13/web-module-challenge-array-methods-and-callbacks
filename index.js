...
function getFinals(info) {
   const allFinals = info.filter((item) => {
...
function getYears(info, cb) {
  return cb(info).map(item => item.Year);   
}
...
function getWinners(info, cb) {
    return cb(info).map(item => item['Home Team Goals'] > item['Away Team Goals'] ?
...
function getWinnersByYear(info, getFinalsCB, getYearsCB, getWinnersCB) {
    const winners = getWinnersCB(info, getFinalsCB);
    const years = getYearsCB(info, getFinalsCB);
...
function getAverageGoals(info) {
    const averageHomeGoals = info.reduce((acc, item) => {
        return acc + item['Home Team Goals'] + item['Away Team Goals'];
    }, 0);
    return (averageHomeGoals / info.length).toFixed(2);
}
